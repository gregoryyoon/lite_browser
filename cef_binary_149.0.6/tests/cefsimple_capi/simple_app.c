...
void CEF_CALLBACK browser_process_handler_on_context_initialized(
  ...
  if (use_views) {
    ...
    // Use pre-created client_handler for the UI browser view
    simple_handler_t* ui_client_handler = client_handler;

    // Create the UI browser view delegate (is_ui_view = 1, preferred_height = 80)
    simple_browser_view_delegate_t* ui_view_delegate =
        browser_view_delegate_create(runtime_style, 1, 80);
    CHECK(ui_view_delegate);

    // Create the UI browser view
    cef_browser_view_t* ui_browser_view = cef_browser_view_create(
        &ui_client_handler->client, &ui_url, &browser_settings, NULL, NULL,
        &ui_view_delegate->delegate);
    cef_string_clear(&ui_url);

    // Create the Content browser view delegate (is_ui_view = 0, preferred_height = 0)
    simple_browser_view_delegate_t* content_view_delegate =
        browser_view_delegate_create(runtime_style, 0, 0);
    CHECK(content_view_delegate);

    // Create startup URL string
    cef_string_t content_url = {};
    cef_string_from_ascii(g_startup_url, strlen(g_startup_url), &content_url);

    // Create content handler (uses Alloy style if required)
    simple_handler_t* content_client_handler = simple_handler_create(use_alloy_style);

    // Create the Content browser view
    cef_browser_view_t* content_browser_view = cef_browser_view_create(
        &content_client_handler->client, &content_url, &browser_settings, NULL, NULL,
        &content_view_delegate->delegate);
    cef_string_clear(&content_url);

    LogMsg("ui_browser_view=%p, content_browser_view=%p\n", ui_browser_view, content_browser_view);

    if (ui_browser_view && content_browser_view) {
      ...
      simple_window_delegate_t* window_delegate = window_delegate_create(
          ui_browser_view, content_browser_view, runtime_style, initial_show_state);
