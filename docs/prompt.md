...
### 1. 동적 DPI 배율 획득 및 탭바 높이 비례 계산
- **DPI 비율 산출**: `GetDpiForWindow(hwnd)` API를 통해 현재 윈도우의 DPI 값을 읽어옵니다. 기준 배율 비율 `scale = (double)dpi / 96.0;`을 구합니다.
- **DPI 비례 높이 연산**: 주소창 HTML의 CSS 바디 높이 규격을 엣지/크롬 황금비와 일치하도록 **`72px`**로 최적화하고, 배율을 곱한 물리 픽셀 크기(`ui_height = (int)(72.0 * scale)`)를 동적으로 계산합니다.

### 2. 하위 브라우저 정렬 일원화
- 메인 윈도우의 `WM_SIZE` 메시지 처리기에서 `ui_height`를 기반으로 `content_y = ui_height + 1` 및 `content_h = height - content_y - 1` 좌표를 동적으로 할당하여 자식 창들을 부드럽게 재배치합니다.
- `simple_app.c` 의 최초 창 생성부(`create_browser_window`), 탭 분리 생성부(`create_browser_window_for_detached`) 및 `simple_handler.c` 의 새 탭 추가(`new-tab`), view-source (`view-source`), 탭 전환 등의 모든 Win32 자식 브라우저 Bounds 좌표 배치를 이 동적 스케일 계산에 맞춰 정렬하여, 디스플레이 배율이 변해도 단 1픽셀의 잘림이나 영역 가림 없이 반응형으로 레이아웃이 정확히 유지되도록 보장합니다.

# 링크 및 팝업 가로채기를 통한 새 탭 전환

### 1. 팝업 자식창 전환 및 새 탭 연동
- **`on_before_popup` 자식창 개조**: `cef_life_span_handler_t` 의 `on_before_popup` 콜백을 구현하여 팝업 이벤트 발생 시 `windowInfo` 구조체의 style을 `WS_CHILD`로 개조하고 부모 윈도우 핸들(`win_ctx->main_hwnd`) 및 Bounds 좌표를 직접 매핑한 후, 새 탭용 `content_handler` 클라이언트를 `*client` 매개변수에 주입하고 `0` (false)을 리턴하여 CEF가 내부적으로 자식 창으로 직접 팝업 브라우저를 생성하도록 허용했습니다.
- **세션 타겟 유지 및 덮어쓰기 해결**: 팝업 자식창 생성 시 **`no_javascript_access = 1`**을 설정하여 독자적인 Browsing Context Group 세션으로 분리했습니다. 또한, 브라우저 비동기 생성 과정에서 여러 탭을 연달아 광클하여 로딩이 겹치는 경우, 선착순으로 슬롯을 매핑하여 탭 꼬임 및 덮어쓰기가 일어나던 레이스 컨디션을 해결하기 위해 `tab_info_t`에 **`void* tab_handler`** 필드를 신설하고 `on_after_created` 시점에 생성한 handler 인스턴스 주소를 정밀 비교 매칭하여 자신에게 할당된 정확한 탭 슬롯을 100% 매핑하도록 설계했습니다.
- **새 탭 개설 및 활성 인덱스 동기화**: 팝업 인입 즉시 `active_tab_index`를 새 탭 인덱스로 설정하고 `update_ui_tabs`를 호출하여 탭바 UI 뼈대가 활성 상태로 지연 없이 동기화 렌더링되게 설계했습니다.

### 2. 백그라운드 로드 버퍼링을 통한 깜빡임 해결 및 즉시 반응성 확보
- **지연 노출(SW_HIDE) 관리**: 새 탭 생성 시 내용물이 전혀 준비되지 않은 디폴트 상태의 흰색 브라우저 윈도우가 즉각 노출(`SW_SHOW`)되어 깜빡거리는 현상을 막기 위해, 생성 시점에는 HWND를 `SW_HIDE`로 숨겨두고 이전 탭 화면을 그대로 띄워 둡니다.
- **로딩 시작 시점 즉시 스위칭**: `cef_load_handler_t` 의 `on_loading_state_change` 콜백에서 로딩 시작(`isLoading == 1`을 포함한 모든 상태 변경) 신호를 받는 즉시 `tabs[i].is_loaded = 1;` 로 세팅하고, 해당 탭이 활성화되어야 할 active 탭인 경우 즉각 윈도우를 `SW_SHOW` 시키고 기존 탭들을 숨깁니다. 이를 통해 흰색 화면이 번쩍 튀는 극초기 깜빡임은 완벽하게 거르면서도, 로딩 완료까지 한참을 멍하니 기다리지 않고 클릭하자마자 바로 새 탭으로 넘어가 웹 로딩 프로그레스를 체감할 수 있는 완벽한 전이 UX를 확보했습니다.

### 3. JSON 문자열 이스케이프 처리를 통한 타이틀 갱신 및 UI 크래시 해결
- **`EscapeJsonString` 자체 구현**: 탭의 타이틀이나 URL에 뉴스 기사 고유 특수 문자(큰따옴표 `"`, 백슬래시 `\`, 개행문자 등)가 삽입될 시 문법이 파괴된 JSON 배열이 프론트엔드로 수신되어 `JSON.parse` 자바스크립트 크래시를 유발하던 현상을 박멸했습니다.
- 백엔드 C++ 소스에 `EscapeJsonString` 함수를 만들어 탭바 JSON 구성(`update_ui_tabs`) 및 주소창 문자열 매핑(`update_ui_nav_state`) 시 모든 유저 텍스트를 인코딩 이스케이프함으로써 타이틀이 '새 탭'으로 먹통이 되는 버그와 UI 오동작을 완벽 차단했습니다.

### 4. 주소 표시창 최초 포커스 시 전체 텍스트 자동 선택 기능
- **포커스 획득 시 전체 선택**: 주소창에 마우스 포커스가 없던 상태에서 주소창을 마우스로 한 번 클릭하면 텍스트 박스 전체 문자열이 즉각 드래그 선택 상태(`select()`)가 되도록 프론트엔드 `ui/app.js` 이벤트를 설계했습니다.
- `focus`, `mouseup`, `blur` 이벤트를 유기적으로 활용하여 포커스가 이미 얹어져 있는 상태에서의 드래그나 추가 클릭 에디팅 동작을 전혀 방해하지 않고 일반적인 에디팅 기능을 온전히 보장하면서도 포커스가 신규 인입될 때의 시각적 주소 교체 편의성을 완벽 구현했습니다.

### 5. 새 탭 공통 헬퍼화, 상대 위치 삽입 및 Z-order HWND 감춤 리팩토링
- **현재 활성 탭 바로 오른쪽 위치 새 탭 삽입 (Active Tab Relative Insertion)**: 기존에 새 탭이 생성되면 무조건 전체 탭 목록의 맨 우측 끝에 추가되던 방식에서 벗어나, 크롬/엣지 표준 UX에 맞춰 **현재 보고 있는 활성 탭의 바로 오른쪽(`active_tab_index + 1`) 위치에 새 탭이 즉시 삽입(Insert)**되도록 `CreateNewTab` 공통 함수의 탭 시프트(Shift) 알고리즘을 갱신했습니다. 이 로직은 `+` 버튼/메뉴를 통한 새 탭 생성뿐만 아니라, 웹페이지 내 링크 클릭(`target="_blank"`, `window.open` 팝업 차단 후 우회 등)에 따른 새 탭 개설에도 100% 동일하게 한 방에 일괄 적용됩니다.
- **HWND 다중 숨김(Z-order 감춤)**: 새 브라우저 생성 완료(`on_after_created`) 및 탭 전환(`switch-tab`) 상황에서 기존 활성 탭 하나만 숨기던 방식 대신, 활성화되는 대상 탭을 제외한 **나머지 모든 탭들을 루프 순회하며 명시적으로 `SW_HIDE` 처리**하여 백그라운드 탭들이 전면 윈도우 뒤에 숨겨진 채 겹쳐 보이거나 노출되는 현상을 완벽 차단했습니다.
